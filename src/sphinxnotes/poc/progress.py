...
def generate_pie_svg(pct: int, fg_colors: list[str], bg_color1: str, bg_color2) -> str:
    ...
    if pct == 100:
        svg.append(
            f'<circle cx="{RADIUS}" cy="{RADIUS}" r="{RADIUS}" fill="{fg_color}"/>'
        )
    elif pct > 0:
    ...
    svg.append(
        f'<circle cx="{RADIUS}" cy="{RADIUS}" r="{RADIUS / 2}" fill="{bg_color1}"/>'
    )
    svg.append('</svg>')
    svg = ''.join(svg)
    return svg
...
def setup(app: Sphinx):
    app.add_config_value(
        'progress_fg_colors', DEFAULT_FG_COLORS, 'html', types=list[str]
    )
